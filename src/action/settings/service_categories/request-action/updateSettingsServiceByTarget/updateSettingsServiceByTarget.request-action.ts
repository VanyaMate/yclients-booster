...
export const updateSettingsServiceByTargetRequestAction = async function (bearer: string, clientId: string, clientData: SettingsServiceData, targetData: SettingsServiceData, logger?: ILogger): Promise<SettingsServiceData> {
    ...
    const composeData: SettingsServiceItemUpdateData = {
        ...
        image                   : targetData.image_group?.images?.basic?.path
                                  ? await base64ImageLoad(targetData.image_group.images.basic.path)
                                  : undefined,
        ...
        resources               : clientData.resources.map((resource) => resource),
