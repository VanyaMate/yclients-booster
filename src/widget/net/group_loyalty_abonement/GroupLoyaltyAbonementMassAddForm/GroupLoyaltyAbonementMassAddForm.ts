...
import { CheckboxWithLabel } from "@/shared/input/CheckboxWithLabel/CheckboxWithLabel.ts";
import { Logger } from "@/entity/logger/Logger/Logger.ts";
import { ProgressBar } from "@/shared/progress/ProgressBar/ProgressBar.ts";
import { GroupLoyaltyAbonementActionComponent } from "@/widget/net/group_loyalty_abonement/GroupLayaltyAbonementActionComponent/GroupLoyaltyAbonementActionComponent.ts";
import { PromiseSplitter } from "@/service/PromiseSplitter/PromiseSplitter.ts";
import {
	PROMISE_SPLITTER_MAX_REQUESTS,
	PROMISE_SPLITTER_MAX_RETRY,
} from "@/service/PromiseSplitter/const/const.ts";
import { getGroupLoyaltyAmonements } from "@/action/net/group-loyalty-abonement/getGroupLoyaltyAmonements.ts";
...
export class GroupLoyaltyAbonementMassAddForm extends Component<HTMLDivElement> {
		...
		return categories.filter(Boolean).map((category) => {
			...
			if (categoryId) {
				...
				if (isNull(visitNumber)) {
					...
					throw new Error(
						`"${amount}" внутри "${category}" внутри "${value}" является не валидным количеством`,
					);
				}
			...
			throw new Error(
				`"${category}" внутри "${value}" не является валидной категорией`,
			);
		});
	}
		...
		return items.filter(Boolean).map((item) => {
			...
			if (categoryId && serviceId) {
				...
				if (isNull(visitNumber)) {
					...
					throw new Error(
						`"${amount}" внутри "${item}" внутри "${value}" является не валидным количеством`,
					);
				}
			...
			throw new Error(
				`"${item}" внутри "${value}" не является валидой услугой`,
			);
		});
	}
	...
	private _getSalonChangeType(
		...
		switch (value) {
			...
			default:
				throw new Error(
					`"${value}" не является валидным значением типа. "нет", "гдепродан", "везде"`,
				);
		}
	}
	...
	private _getYesOrNoBool(value: string): boolean {
		switch (value.trim().toLowerCase()) {
			...
			default:
				throw new Error(
					`"${value}" не валидное значение. Можно только "да" или "нет"`,
				);
		}
	}
